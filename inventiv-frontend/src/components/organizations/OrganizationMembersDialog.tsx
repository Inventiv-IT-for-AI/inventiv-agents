...
export function OrganizationMembersDialog({
  ...
  const fetchMembers = async () => {
    ...
    setLoading(true);
    try {
      // First, set this organization as current to access its members
      ...
      if (!setCurrentRes.ok) {
        const body = await setCurrentRes.json().catch(() => null);
        const code = body?.error || body?.message;
        const msg =
          code === "not_a_member"
            ? "Vous n'êtes pas membre de cette organisation"
            : "Impossible d'accéder aux membres";
        setError(msg);
        setMembers([]);
        return;
      }
      ...
      // Then fetch members
      const res = await apiRequest("/organizations/current/members");
      if (!res.ok) {
  ...
  const setRole = async (memberUserId: string, toRole: OrgRole) => {
    ...
    try {
      ...
      const res = await apiRequest(`/organizations/current/members/${memberUserId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const removeMember = async (memberUserId: string) => {
    ...
    const isSelf = !!currentUserId && currentUserId === memberUserId;
    ...
    try {
      ...
      const res = await apiRequest(`/organizations/current/members/${memberUserId}`, { method: "DELETE" });
      const body = await res.json().catch(() => null);
      if (!res.ok) {
        const code = body?.error || body?.message;
        const msg =
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showCloseButton={false} className="sm:max-w-[720px]">
        <DialogHeader>
          <DialogTitle>Membres & rôles - {organizationName}</DialogTitle>
        </DialogHeader>
            ...
            <div className="border rounded-md overflow-hidden">
              ...
              <div className="divide-y">
                {members.map((m) => {
                  ...
                  const isSelf = !!currentUserId && currentUserId === m.user_id;
                  return (
