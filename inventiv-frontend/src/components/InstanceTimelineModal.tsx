import { X, ChevronDown, ChevronRight, Server, Zap, Cloud, Database, Archive, AlertTriangle } from 'lucide-react';
...
export function InstanceTimelineModal({ open, onClose, instanceId }: InstanceTimelineModalProps) {
    ...
    const fetchInstanceLogs = async () => {
        setLoading(true);
        try {
            const response = await fetch(`http://localhost:8003/action_logs?instance_id=${instanceId}&limit=100`);
            const data = await response.json();
            // Sort by created_at to show chronological order
            const sorted = data.sort((a: ActionLog, b: ActionLog) =>
    ...
    const getStatusIcon = (status: string) => {
        if (status === "success") return <span className="text-green-500 text-2xl">✅</span>;
        if (status === "failed") return <span className="text-red-500 text-2xl">❌</span>;
                ...
                <DialogHeader>
                    <DialogTitle className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground font-mono mt-1">{instanceId}</p>
                        </div>
                    </DialogTitle>
                </DialogHeader>
                        ...
                        {logs.map((log, index) => {
                            ...
                            return (
                                <div key={log.id} className="relative">
                                    {/* Timeline connector */}
                                    {index < logs.length - 1 && (
                                        <div className="absolute left-6 top-16 bottom-0 w-0.5 bg-gradient-to-b from-gray-300 to-transparent h-8"></div>
                                    )}

                                    <Card className={`border-l-4 ${getActionColor(log.action_type)} p-4 shadow-sm hover:shadow-md transition-shadow`}>
                                        <div className="flex items-start gap-4">
                                            {/* Icon */}
                                            <div className="flex-shrink-0">
                                                <div className={`p-3 rounded-full ${getActionColor(log.action_type)} border-2`}>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center justify-between mb-2">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <h3 className="font-bold text-lg">{log.action_type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</h3>
                                                            <p className="text-sm text-muted-foreground">
                                                                {formatTime(log.created_at)} • Component: <span className="font-semibold">{log.component}</span>
                                                            </p>
                                                        </div>
                                                    </div>
                                                ...
                                                {hasMetadata && (
                                                    <div className="mt-3">
                                                        ...
                                                        >
                                                            ...
                                                            <span>Metadata ({Object.keys(log.metadata!).length} fields)</span>
                                                        </button>
                                                        {isExpanded && (
