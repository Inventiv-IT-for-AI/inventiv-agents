...
-- Secrets should be seeded from /run/secrets (never committed).
--
-- We store the Scaleway secret key as encrypted+base64 text in provider_settings.value_text
-- using pgcrypto: encode(pgp_sym_encrypt(secret, passphrase), 'base64').
--
-- Decryption is done in-app using pgp_sym_decrypt(decode(value_text,'base64'), passphrase).
