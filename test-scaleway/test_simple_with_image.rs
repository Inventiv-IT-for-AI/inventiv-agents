...
/// Test simple : créer instance avec image qui a sbs_snapshot - Scaleway devrait créer Block Storage automatiquement

#[tokio::main]
async fn main() -> Result<()> {
    println!("🧪 Testing simple instance creation with image (sbs_snapshot)\n");
    ...
    // Step 1: Create Block Storage empty (200GB) - Scaleway should populate it from image snapshot
    println!("[1/5] Creating Block Storage volume (200GB empty - will be populated from image)...");
    let volume_name = format!("test-l4-simple-{}", uuid::Uuid::new_v4());
    let volume_id = create_block_storage_empty(
        &client,
        &headers,
        &zone,
        &project_id,
    ...
    let access_key = env::var("SCALEWAY_ACCESS_KEY")
        .context("SCALEWAY_ACCESS_KEY required for CLI attachment")?;
    let org_id = env::var("SCALEWAY_ORGANIZATION_ID")
        .context("SCALEWAY_ORGANIZATION_ID required for CLI attachment")?;

    // Step 2: Create instance WITHOUT volumes first
    println!("[2/5] Creating instance WITHOUT volumes (will attach Block Storage after)...");
    let instance_name = format!("test-l4-simple-{}", uuid::Uuid::new_v4());
    ...
    let create_body = json!({
        ...
        "boot_type": "local"
        // NO volumes field - attach Block Storage via CLI after
    });
    ...
    // Step 3: Attach Block Storage via CLI BEFORE starting
    println!("[3/5] Attaching Block Storage via CLI (before starting instance)...");
    ...
    // Check volumes
    println!("   Checking volumes after attachment...");
...
async fn create_block_storage_empty(
    client: &reqwest::Client,
    headers: &reqwest::header::HeaderMap,
    zone: &str,
    project_id: &str,
