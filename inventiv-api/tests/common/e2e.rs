...
impl TestClient {
    pub fn new() -> Self {
        let api_url =
            std::env::var("TEST_API_URL").unwrap_or_else(|_| "http://localhost:8003".to_string());

        let admin_email = std::env::var("TEST_ADMIN_EMAIL").unwrap_or_else(|_| {
        ...
        let admin_password = std::env::var("TEST_ADMIN_PASSWORD").unwrap_or_else(|_| {
    ...
    pub async fn login(&self) -> String {
        ...
        body.get("session_token")
            ...
            .unwrap_or_else(|| {
                panic!(
                    "No session token found in login response. Response: {:?}",
                    body
                )
            })
    }
