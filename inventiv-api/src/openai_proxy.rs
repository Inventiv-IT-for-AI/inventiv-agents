...
pub async fn proxy_to_worker(
    ...
    eprintln!("[OPENAI_PROXY] [{}] START: path={}, body_size={}", correlation_id, path, body.len());
    
    let v: serde_json::Value = match serde_json::from_slice(&body) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("[OPENAI_PROXY] [{}] ERROR: Invalid JSON body: {}", correlation_id, e);
            return (
    ...
    eprintln!("[OPENAI_PROXY] [{}] REQUEST: model={:?}, stream={}", 
        correlation_id, 
        requested_model,
        v.get("stream").and_then(|b| b.as_bool()).unwrap_or(false)
    );
    
    let model_id = match worker_routing::resolve_openai_model_id(&state.db, requested_model, user.as_ref()).await {
        Ok(m) => {
            eprintln!("[OPENAI_PROXY] [{}] MODEL_RESOLVED: model_id={}", correlation_id, m);
            m
        },
        Err(e) => {
            eprintln!("[OPENAI_PROXY] [{}] ERROR: Model resolution failed", correlation_id);
    ...
    let Some((instance_id, base_url)) =
        worker_routing::select_ready_worker_for_model(&state.db, &model_id, sticky.as_deref()).await
    else {
        eprintln!("[OPENAI_PROXY] [{}] ERROR: No ready worker found for model_id={}", correlation_id, model_id);
    ...
    eprintln!("[OPENAI_PROXY] [{}] WORKER_SELECTED: instance_id={}, target={}, stream={}", 
    ...
    eprintln!("[OPENAI_PROXY] [{}] UPSTREAM_REQUEST: sending POST to {}", correlation_id, target);
        ...
        .await
    {
        Ok(r) => {
            ...
            eprintln!("[OPENAI_PROXY] [{}] UPSTREAM_RESPONSE: status={}, elapsed_ms={}", 
                correlation_id, r.status(), elapsed.as_millis());
            r
        },
        Err(e) => {
            let elapsed = start_time.elapsed();
            ...
            worker_routing::bump_runtime_model_counters(&state.db, &model_id, false).await;
            metrics::update_instance_request_metrics(&state.db, instance_id, false, None, None, None).await;
...
async fn handle_streaming_response(
    ...
    eprintln!("[OPENAI_PROXY] [{}] STREAMING_START: status={}, content_type={:?}", 
        correlation_id, 
        status,
        resp_headers.get(axum::http::header::CONTENT_TYPE).and_then(|h| h.to_str().ok())
    );
    
    let success = status.is_success();
    worker_routing::bump_runtime_model_counters(&state.db, model_id, success).await;
    metrics::update_instance_request_metrics(&state.db, instance_id, success, None, None, None).await;
    ...
    tokio::spawn(async move {
        ...
        eprintln!("[OPENAI_PROXY] [{}] STREAM_BUFFER_SIZE: {} bytes", correlation_id_for_tokens, buffer.len());
        if buffer.len() > 0 && buffer.len() < 2000 {
            eprintln!("[OPENAI_PROXY] [{}] STREAM_BUFFER_PREVIEW: {}", correlation_id_for_tokens, 
                text.chars().take(500).collect::<String>());
        }
        let (input_tokens, output_tokens, total_tokens) = metrics::parse_tokens_from_sse_stream(&text);
        
        if input_tokens.is_some() || output_tokens.is_some() || total_tokens.is_some() {
            eprintln!("[OPENAI_PROXY] [{}] STREAM_TOKENS_EXTRACTED: input={:?}, output={:?}, total={:?}", 
            ...
            if let Some(model_uuid) = metrics::resolve_model_uuid(&db_for_tokens, &model_id_for_tokens).await {
...
async fn handle_non_streaming_response(
    ...
    eprintln!("[OPENAI_PROXY] [{}] NON_STREAMING: reading response body", correlation_id);
    let bytes = match upstream.bytes().await {
        Ok(b) => {
            eprintln!("[OPENAI_PROXY] [{}] NON_STREAMING_SUCCESS: body_size={}", correlation_id, b.len());
            b
        },
        Err(e) => {
            eprintln!("[OPENAI_PROXY] [{}] NON_STREAMING_ERROR: {}", correlation_id, e);
            let success = false;
            worker_routing::bump_runtime_model_counters(&state.db, model_id, success).await;
            metrics::update_instance_request_metrics(&state.db, instance_id, success, None, None, None).await;
            return (
    ...
    let (input_tokens, output_tokens, total_tokens) = if success {
        match serde_json::from_slice::<serde_json::Value>(&bytes) {
            Ok(json) => {
                ...
                eprintln!("[OPENAI_PROXY] [{}] TOKENS_EXTRACTED: input={:?}, output={:?}, total={:?}", 
                    correlation_id, tokens.0, tokens.1, tokens.2);
                tokens
            },
            Err(e) => {
                eprintln!("[OPENAI_PROXY] [{}] TOKEN_EXTRACTION_ERROR: failed to parse JSON: {}", correlation_id, e);
                (None, None, None)
            }
        }
    } else {
